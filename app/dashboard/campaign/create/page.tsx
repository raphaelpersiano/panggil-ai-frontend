...
function CreateCampaignForm() {
  ...
  const [pageRows, setPageRows] = useState<Lead[]>([]);
  const [matchedCount, setMatchedCount] = useState(0);
  const [totalPages, setTotalPages] = useState(1);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [leadMode, setLeadMode] = useState<"explicit" | "filter">("filter");
  ...
  useEffect(() => {
    ...
    async function loadLeads() {
      ...
      setLoadError(null);
      try {
        const response = await listLeads({ type: initialType, page, limit: PAGE_SIZE, status: statusFilter, search: searchQ, sortBy: sortFilter });
        if (cancelled) return;
  ...
  }, [initialType, page, searchQ, statusFilter, sortFilter]);

  const selectedCount = selectedIds.size;
  const pageSelectedCount = useMemo(() => pageRows.filter((lead) => selectedIds.has(lead.id)).length, [pageRows, selectedIds]);
  const allVisibleSelected = pageRows.length > 0 && pageRows.every((lead) => selectedIds.has(lead.id));
  const filterSummary = useMemo(() => {
    const parts: string[] = [];
    if (statusFilter !== "all") parts.push(`status ${STATUS_LABELS[statusFilter].toLowerCase()}`);
    if (searchQ.trim()) parts.push(`kata kunci “${searchQ.trim()}”`);
    return parts.length > 0 ? parts.join(" · ") : "semua leads";
  }, [searchQ, statusFilter]);
  const canSubmit = scheduleMode === "immediate"
    ? !!name && (leadMode === "filter" ? matchedCount > 0 : selectedCount > 0)
    : !!name && !!startDate && !!endDate && selectedDays.length > 0 && (leadMode === "filter" ? matchedCount > 0 : selectedCount > 0);
  ...
  function toggleLead(id: string) { setSelectedIds((prev) => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next; }); setLeadMode("explicit"); }
  function toggleSelectVisible() { setSelectedIds((prev) => { const next = new Set(prev); if (allVisibleSelected) pageRows.forEach((lead) => next.delete(lead.id)); else pageRows.forEach((lead) => next.add(lead.id)); return next; }); setLeadMode("explicit"); }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const payload: CreateCampaignPayload = {
        ...
        maxConcurrent: 10,
        leadSelection: leadMode === "filter"
          ? { mode: "filter", filter: { status: statusFilter !== "all" ? statusFilter : undefined, search: searchQ || undefined } }
          : { mode: "explicit", leadIds: Array.from(selectedIds) },
      };
  ...
  return <div className="min-h-screen bg-gray-50">
    <div className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
      <div className="flex items-center gap-3"><button onClick={() => router.back()} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors"><ArrowLeft className="w-4 h-4 text-gray-500" /></button><div><h1 className="text-base font-bold text-gray-900">Buat Campaign Baru</h1><p className="text-xs text-gray-400">Seleksi lead sekarang server-driven dan scalable untuk dataset besar.</p></div></div>
    ...
    <div className="max-w-5xl mx-auto px-8 py-8 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"><button onClick={() => setShowAdvanced(!showAdvanced)} className="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50/60 transition-colors"><div className="flex items-center gap-3"><span className="text-[11px] font-bold text-[#12672a] bg-[#12672a]/10 px-2 py-0.5 rounded-full tracking-wide">03</span><div className="w-6 h-6 rounded-lg bg-[#12672a]/10 flex items-center justify-center"><Filter className="w-3.5 h-3.5 text-[#12672a]" /></div><h2 className="text-sm font-semibold text-gray-800">Pengaturan Panggilan</h2><span className="text-xs text-gray-400">(Opsional)</span></div>{showAdvanced ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}</button>{showAdvanced && <div className="px-6 pb-6 border-t border-gray-50 pt-5"><div className="grid grid-cols-1 sm:grid-cols-3 gap-5"><div><Label hint="Berapa kali mencoba kembali jika panggilan tidak diangkat">Maks. Percobaan Ulang</Label><div className="relative"><Input type="number" min={1} max={10} value={maxRetries} onChange={(e) => setMaxRetries(+e.target.value)} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">kali</span></div></div><div><Label hint="Jeda waktu antar percobaan ulang ke lead yang sama">Interval Percobaan Ulang</Label><div className="relative"><Input type="number" min={15} max={120} value={retryMinutes} onChange={(e) => setRetryMinutes(+e.target.value)} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">menit</span></div></div><div><Label hint="Jumlah panggilan simultan yang dapat dilakukan campaign ini">Maks. Panggilan Bersamaan</Label><div className="relative"><Input type="number" value={10} disabled /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">saluran</span></div></div></div><div className="mt-5 bg-amber-50 border border-amber-200 rounded-xl p-4"><div className="flex items-start gap-2.5"><Lightbulb className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" /><div><p className="text-xs font-semibold text-amber-700 mb-2">Catatan integrasi</p><ul className="space-y-1.5 text-xs text-amber-700/80"><li>• Frontend mengirim <strong>scheduleMode</strong>, <strong>selectedDays</strong>, dan <strong>dayTimes</strong> persis ke backend.</li><li>• Mode <strong>leadSelection.filter</strong> sekarang jadi jalur default untuk dataset besar.</li><li>• Jika backend menolak payload ini, masalahnya ada di kontrak beng, bukan di UI.</li></ul></div></div></div></div>}</div>
      <Section num="04" title="Pilih Leads" icon={Users}>
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr] mb-5">
          <div className="rounded-2xl border border-[#12672a]/20 bg-[#12672a]/5 p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-[#12672a] mb-2">Mode scalable (disarankan)</p>
            <h3 className="text-sm font-semibold text-gray-900 mb-1">Gunakan semua lead yang cocok dengan filter backend</h3>
            <p className="text-xs text-gray-500 mb-3">Frontend hanya menampilkan preview per halaman. Resolusi lead final dilakukan backend saat campaign dibuat.</p>
            <button onClick={() => setLeadMode("filter")} disabled={matchedCount === 0} className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-colors ${leadMode === "filter" ? "bg-[#12672a] text-white" : "bg-white border border-gray-200 text-gray-700 hover:border-[#12672a]/40"} disabled:opacity-50`}>
              <CheckSquare className="w-4 h-4" />
              Pilih semua hasil filter ({matchedCount})
            </button>
            <div className="mt-3 text-xs text-gray-600">Filter aktif: <span className="font-medium">{filterSummary}</span></div>
          </div>

          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2">Mode manual</p>
            <h3 className="text-sm font-semibold text-gray-900 mb-1">Pilih lead satu per satu dari halaman preview</h3>
            <p className="text-xs text-gray-500 mb-3">Cocok untuk kampanye kecil atau kurasi akun prioritas lintas halaman.</p>
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => setLeadMode("explicit")} className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-colors ${leadMode === "explicit" ? "bg-[#12672a] text-white" : "bg-white border border-gray-200 text-gray-700 hover:border-[#12672a]/40"}`}>
                <Square className="w-4 h-4" />
                Gunakan pilihan manual ({selectedCount})
              </button>
              {selectedCount > 0 && <button onClick={clearAll} className="text-xs text-red-500 hover:text-red-700 transition-colors">Hapus semua pilihan manual</button>}
            </div>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-3 mb-4"><div className="relative flex-1 min-w-[180px]"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-300" /><input type="text" placeholder="Cari nama / nomor..." value={searchQ} onChange={(e) => setSearchQ(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#12672a]/30 focus:border-[#12672a] transition-all placeholder:text-gray-300" /></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as LeadStatus | "all")} className="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#12672a]/30 focus:border-[#12672a] bg-white"><option value="all">Semua Status</option>{(Object.keys(STATUS_LABELS) as LeadStatus[]).filter((s) => initialType === "collection" || s !== "promise_to_pay").map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}</select><select value={sortFilter} onChange={(e) => setSortFilter(e.target.value as typeof sortFilter)} className="px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#12672a]/30 focus:border-[#12672a] bg-white"><option value="newest">Terbaru Dibuat</option><option value="oldest">Terlama Dibuat</option><option value="last_call">Terakhir Dihubungi</option></select><span className="ml-auto text-xs font-semibold text-[#12672a] bg-[#12672a]/10 px-2.5 py-1 rounded-full">{leadMode === "filter" ? `Campaign akan memakai ${matchedCount} lead dari backend filter` : `${selectedCount} lead dipilih manual`}</span></div>

        <div className="flex items-center justify-between mb-3 px-1">
          <div className="flex items-center gap-3">
            <button onClick={toggleSelectVisible} disabled={loadingLeads || pageRows.length === 0} className="flex items-center gap-1.5 text-xs font-medium text-[#12672a] hover:text-[#0e5222] transition-colors disabled:opacity-40">
              {allVisibleSelected ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
              {allVisibleSelected ? "Batalkan pilihan halaman ini" : `Pilih semua di halaman ini (${pageRows.length})`}
            </button>
            <span className="text-xs text-gray-400">{pageSelectedCount} terpilih di halaman {page}</span>
          </div>
          <span className="text-xs text-gray-400">Preview {pageRows.length} dari {matchedCount} lead</span>
        </div>

        <div className="border border-gray-200 rounded-xl overflow-hidden"><table className="w-full"><thead><tr className="bg-gray-50 border-b border-gray-200"><th className="w-10 px-4 py-2.5" /><th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wide">Nama</th><th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">Nomor HP</th><th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wide">Status</th><th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden md:table-cell">Dibuat</th></tr></thead><tbody className="divide-y divide-gray-100">{loadingLeads ? <tr><td colSpan={5} className="text-center py-10 text-sm text-gray-400">Memuat leads...</td></tr> : pageRows.length === 0 ? <tr><td colSpan={5} className="text-center py-10 text-sm text-gray-400">Tidak ada leads yang sesuai filter</td></tr> : pageRows.map((lead) => { const checked = selectedIds.has(lead.id); return <tr key={lead.id} onClick={() => toggleLead(lead.id)} className={`cursor-pointer transition-colors ${checked ? "bg-[#12672a]/5" : "hover:bg-gray-50/60"}`}><td className="px-4 py-3 text-center"><div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all mx-auto ${checked ? "bg-[#12672a] border-[#12672a]" : "border-gray-300"}`}>{checked && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 10 10"><path d="M1.5 5l2.5 2.5 4.5-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div></td><td className="px-4 py-3"><p className="text-sm font-medium text-gray-800">{lead.name}</p><p className="text-xs text-gray-400 sm:hidden">{lead.mobile}</p></td><td className="px-4 py-3 hidden sm:table-cell"><span className="text-sm text-gray-500 tabular-nums">{lead.mobile}</span></td><td className="px-4 py-3"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[lead.status]}`}>{STATUS_LABELS[lead.status]}</span></td><td className="px-4 py-3 hidden md:table-cell"><span className="text-xs text-gray-400">{fmtDate(lead.createdAt)}</span></td></tr>; })}</tbody></table></div>

        <div className="flex items-center justify-between px-1 pt-4">
          <div className="flex items-start gap-1.5 text-xs text-gray-400 max-w-2xl"><Info className="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>Preview ini selalu paginated. Frontend tidak lagi memuat semua leads ke memory. Untuk dataset besar, gunakan mode filter agar backend yang menyelesaikan target campaign akhir.</span></div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs text-gray-400">Halaman {page} dari {totalPages}</span>
            <button onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1 || loadingLeads} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">← Prev</button>
            <button onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages || loadingLeads} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Next →</button>
          </div>
        </div>
      </Section>
      <div className="h-4" />
